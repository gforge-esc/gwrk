...
export function parseGitLog(raw) {
    ...
    for (const block of blocks) {
        ...
        if (!hash || !timestamp)
            continue;
        const parsed = { hash, timestamp, files: [] };
 ...
 */
export function bucketByWeek(commits, defaultBranch) {
    if (commits.length === 0)
        return [];
 ...
 */
export function scanSpecProgress(projectRoot) {
    let totalSpecs = 0;
    let totalPlans = 0;
 ...
 */
export function generatePulseReport(config) {

...
| 10 | T | **Compression needs SP data.** `gwrk measure compression --all` returns 0.0x everywhere because no specs have SP values. Compression engine works; input is the gap. Leading indicators shipped (PR #74) but LOC-derived SP fallback (F007 P4: T014-T016) still needs shipping. | ~2 hrs | F007 P4 tasks ready. `gwrk ship 007 4` with correct task scope. |

...
](/r/LocalLLaMA/?f=flair_name%3A%22Discussion%22)
...
tools: \[search\_web, read\_file, write\_file, run\_code, send\_email, ...\]
...
Function-calling approach (3 tool calls):
  1. read\_file(path="/var/log/app.log") → returns entire file
  2. search\_text(text=<entire file>, pattern="ERROR") → returns matching lines
  3. count\_lines(text=<matched lines>) → returns number
...
\[error\] memory: usage: memory search <query> \[-t topic\_id\] \[-k keyword\]

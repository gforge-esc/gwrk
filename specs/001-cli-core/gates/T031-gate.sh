...
grep -q 'export function computeCompression' src/engine/compression.ts \
  || { echo "FAIL: T031 — src/engine/compression.ts missing 'computeCompression'" >&2; exit 1; }

grep -q 'export function generateSummary' src/engine/compression.ts \
  || { echo "FAIL: T031 — src/engine/compression.ts missing 'generateSummary'" >&2; exit 1; }

...
Bootstrap the `gwrk` CLI from a greenfield TypeScript project. Deliver: project scaffolding (`init`), agent dispatch wrappers (`specify`, `plan`, `analyze`, `effort`), the flat-file task tracking engine (`tasks generate`, `tasks list`, `tasks next`, `tasks done`), Hard Gate enforcement, JSONL history logging, and Zod-validated configuration. This is Phase 1 of the master build plan — the keystone that everything else depends on.
...
Bootstrap the TypeScript project infrastructure and deliver the `gwrk init` command that scaffolds a new project with `.agent/`, `.specify/`, `specs/`, and `.gwrkrc.json`.
...
- `src/commands/init.ts` (NEW: `gwrk init` — scaffold directories, copy templates, create `.gwrkrc.json`)
...
- `src/utils/state.ts` (NEW: Read/write `tasks.json` with Zod validation, atomic writes)
...
- `src/utils/history.ts` (NEW: Append to `.gwrk/history.jsonl` on every state transition)

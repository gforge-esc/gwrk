...
As a Principal Engineer, I want to run `gwrk measure pulse` so that I see a productivity snapshot across all tracked repositories — published LOC on main, draft LOC on feature/dev branches, weekly velocity trends — without leaving my terminal.

**Implements**: FR-001, FR-003

**Independent Test**: Add two repos to `.gwrkrc.json`, run `gwrk measure pulse`, verify output contains both repo summaries.

**Acceptance Scenarios**:
1. **Given** a `.gwrkrc.json` with two tracked repos that contain git history, **When** the user runs `gwrk measure pulse --json`, **Then**:
   - `gwrk measure pulse --json | jq -e '.repositories | length == 2'` exits 0
   - `gwrk measure pulse --json | jq -e '.repositories[0].mainLoc > 0'` exits 0
2. **Given** a `.gwrkrc.json` with zero tracked repos, **When** the user runs `gwrk measure pulse`, **Then**:
   - `gwrk measure pulse 2>&1 | grep -q 'No repositories tracked'` exits 0

### US-002 - Historical Scan (Priority: P0)
As a Principal Engineer, I want to run `gwrk measure pulse scan [path]` on any existing git repository so that I get a full historical productivity snapshot with weekly LOC buckets back to the repo's creation — without any gwrk-specific setup in that repo.

**Implements**: FR-002, FR-004

**Independent Test**: Run `gwrk measure pulse scan` against a known repo and verify weekly buckets are generated.

**Acceptance Scenarios**:
1. **Given** a git repository at `/tmp/test-repo` with commits spanning 4 weeks, **When** the user runs `gwrk measure pulse scan /tmp/test-repo --json`, **Then**:
   - `gwrk measure pulse scan /tmp/test-repo --json | jq -e '.weeklyBuckets | length >= 4'` exits 0
   - `gwrk measure pulse scan /tmp/test-repo --json | jq -e '.mainLoc >= 0'` exits 0
2. **Given** a path `/tmp/not-a-repo` that is not a git repo, **When** the user runs `gwrk measure pulse scan /tmp/not-a-repo`, **Then**:
   - `gwrk measure pulse scan /tmp/not-a-repo 2>&1 | grep -q 'Not a git repository'` exits 0
...
**Acceptance Scenarios**:
1. **Given** a repo with 100 LOC on `main` and 50 LOC on `feature/foo`, **When** running `gwrk measure pulse scan <path> --json`, **Then**:
   - `gwrk measure pulse scan <path> --json | jq -e '.mainLoc == 100'` exits 0
   - `gwrk measure pulse scan <path> --json | jq -e '.draftLoc == 50'` exits 0
...
**Acceptance Scenarios**:
1. **Given** a repo with commits in weeks W1, W2, W3, **When** running `gwrk measure pulse scan <path> --json`, **Then**:
   - `gwrk measure pulse scan <path> --json | jq -e '.weeklyBuckets | length >= 3'` exits 0
   - `gwrk measure pulse scan <path> --json | jq -e '.weeklyBuckets[0] | has("weekStart") and has("totalMain") and has("added") and has("deleted")'` exits 0
...
**Independent Test**: Create a project with `specs/` containing some specs, run `gwrk measure pulse`, verify spec counts appear.

**Acceptance Scenarios**:
1. **Given** a project with `specs/001-cli-core/spec.md` and `specs/002-build-server/spec.md`, **When** running `gwrk measure pulse --json`, **Then**:
   - `gwrk measure pulse --json | jq -e '.specProgress.totalSpecs == 2'` exits 0
...
**Independent Test**: Run `gwrk measure pulse --json` and verify valid JSON is returned.

**Acceptance Scenarios**:
1. **Given** a tracked repo, **When** running `gwrk measure pulse --json`, **Then**:
   - `gwrk measure pulse --json | jq -e '.'` exits 0
2. **Given** a repo path, **When** running `gwrk measure pulse scan <path> --json`, **Then**:
   - `gwrk measure pulse scan <path> --json | jq -e '.'` exits 0

### US-007 - Performance on Large Repos (Priority: P1)
As a Principal Engineer, I want `gwrk measure pulse scan` to complete within 60 seconds for repositories with up to 50,000 commits so that it remains practical for real-world projects.
...
**Acceptance Scenarios**:
1. **Given** a repo where the default branch is `master`, **When** running `gwrk measure pulse scan <path> --json`, **Then**:
   - `gwrk measure pulse scan <path> --json | jq -e '.defaultBranch == "master"'` exits 0
2. **Given** a repo where the default branch is `main`, **When** running `gwrk measure pulse scan <path> --json`, **Then**:
   - `gwrk measure pulse scan <path> --json | jq -e '.defaultBranch == "main"'` exits 0
...
- **FR-001**: System MUST provide a `gwrk measure pulse` command that generates a PulseSnapshot across all repositories listed in `.gwrkrc.json` under `pulse.repos` and renders it as a formatted terminal table. (Implements: US-001)
- **FR-002**: System MUST provide a `gwrk measure pulse scan [path]` command that walks the git log of any repository at the given path and generates a PulseSnapshot with weekly LOC buckets from the first commit to HEAD. (Implements: US-002)
...
- **FR-006**: System MUST support a `--json` flag on both `gwrk measure pulse` and `gwrk measure pulse scan` that outputs the full PulseSnapshot as structured JSON to stdout. (Implements: US-006)
- **FR-007**: System MUST complete a `gwrk measure pulse scan` of a repository with ≤ 50,000 commits in under 60 seconds. (Implements: US-007)
...
- **TC-001**: Determinism — Given the same git history, `gwrk measure pulse scan` MUST produce identical `weeklyBuckets` and LOC counts.
- **TC-002**: Air-Gapped — Pulse performs no network calls. All data derived from the local git log.
...
- **TR-004**: `src/commands/pulse.test.ts` — Unit test `gwrk measure pulse` command: mock the engine, verify it reads `pulse.repos` from config and invokes the scanner for each repo. Vitest. (FR-001)
- **TR-005**: `src/commands/pulse.test.ts` — Unit test `gwrk measure pulse scan` command: verify it validates the path argument, calls the engine, and outputs JSON when `--json` is passed. Vitest. (FR-002, FR-006)
...
- **SC-001**: `gwrk measure pulse scan <any-git-repo>` generates a valid PulseSnapshot JSON with no prior gwrk setup in the target repo.
- **SC-002**: `gwrk measure pulse` aggregates across multiple repos and displays a terminal table.
...
- **VR-001**: E2E test: create a temp git repo with commits on `main` and a feature branch across 3 weeks → run `gwrk measure pulse scan <path> --json` → verify JSON has correct `mainLoc`, `draftLoc`, and 3+ weekly buckets.
- **VR-002**: Negative test: run `gwrk measure pulse scan /tmp/not-a-repo` → verify exit code 1 and stderr contains `Not a git repository`.
- **VR-003**: Config test: remove `pulse.repos` from `.gwrkrc.json` → run `gwrk measure pulse` → verify exit code 1 and stderr contains configuration error.
- **VR-004**: Determinism test: run `gwrk measure pulse scan <path> --json` twice → `diff` the outputs → they MUST be identical.

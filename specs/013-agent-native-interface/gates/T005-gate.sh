...
grep -q 'CommandOutput\|createOutput' src/commands/tasks.ts

# Assertion #2: status.ts uses CommandOutput
grep -q 'CommandOutput\|createOutput' src/commands/status.ts

# Assertion #3: E2E — tasks list --format json produces valid JSON
gwrk tasks list 000-tdd-infrastructure --format json 2>/dev/null | jq . > /dev/null || { echo "FAIL: tasks list --format json not valid JSON"; exit 1; }

# Assertion #4: E2E — status --format json produces valid JSON
gwrk status --format json 2>/dev/null | jq . > /dev/null || { echo "FAIL: status --format json not valid JSON"; exit 1; }

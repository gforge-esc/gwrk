...
grep -q 'export.*discoverProject\|export.*function discoverProject' src/engine/discover.ts
...
grep -q 'git status\|git branch\|git log' src/engine/discover.ts
...
if grep -q "localhost:18790\|127\.0\.0\.1:18790" src/engine/discover.ts 2>/dev/null; then

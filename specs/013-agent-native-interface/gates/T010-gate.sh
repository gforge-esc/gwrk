...
gwrk project specs --format json 2>/dev/null | jq -e '.[0].id' > /dev/null || { echo "FAIL: project specs --format json invalid"; exit 1; }

# Assertion #4: E2E — project gates --format json returns array
gwrk project gates --format json 2>/dev/null | jq -e 'type' > /dev/null || { echo "FAIL: project gates --format json invalid"; exit 1; }

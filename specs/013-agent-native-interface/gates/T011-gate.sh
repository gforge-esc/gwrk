...
HELP=$(gwrk gate-check --help 2>/dev/null || true)
echo "$HELP" | grep -qi 'exit code' || { echo "FAIL: gate-check --help missing exit codes"; exit 1; }

# Assertion #2: gate-check --help includes command type
HELP=$(gwrk gate-check --help 2>/dev/null || true)
echo "$HELP" | grep -qiE 'type:|verifier|query|mutator' || { echo "FAIL: gate-check --help missing command type"; exit 1; }

# Assertion #3: tasks --help mentions --format json
HELP=$(gwrk tasks --help 2>/dev/null || true)
echo "$HELP" | grep -q '\-\-format' || { echo "FAIL: tasks --help missing --format"; exit 1; }

# Assertion #4: ship --help declares mutation scope
HELP=$(gwrk ship --help 2>/dev/null || true)
echo "$HELP" | grep -qi 'mutat' || { echo "FAIL: ship --help missing mutation declaration"; exit 1; }

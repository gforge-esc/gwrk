...
vi.mock('./commands/measure.js', () => ({ measureCommand: new Command('measure') }));
...
describe('FR-001 / FR-004: CLI Command Registration', () => {
  it('US-001 / US-003: registers Foxtrot Charlie pillar commands', async () => {
    const parseSpy = vi.spyOn(Command.prototype, 'parse').mockImplementation(() => { return {} as unknown as ReturnType<typeof Command.prototype.parse>; });
    ...
    const eliminated = ['run', 'metrics', 'implement', 'pulse', 'specify', 'plan', 'analyze', 'server', 'status', 'new'];

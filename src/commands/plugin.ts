 ...
 */
export async function installPlugin(sourcePath: string, options: { force?: boolean } = {}) {
  const manifestPath = path.join(sourcePath, "manifest.yaml");
  let rawManifest: string;
  try {
    rawManifest = await fs.readFile(manifestPath, "utf-8");
  } catch (e) {
    throw new Error(`No manifest.yaml found in ${sourcePath}. A valid plugin requires manifest.yaml.`);
  }
  ...
  const targetTypeDir = manifest.type === 'skill' ? 'skills' : 
                        ...
                        manifest.type === 'workflow' ? 'workflows' : 
                        `${manifest.type}s`;
  ...
  if (stats && stats.isDirectory() && !options.force) {
    throw new Error(`Plugin '${manifest.name}' already installed. Use --force to overwrite.`);
  }
 ...
 */
export async function removePlugin(name: string, options: { force?: boolean } = {}) {
  const loader = new PluginLoader();
  const plugin = await loader.resolvePlugin(name);
  
  // Dependency check (FR-004)
  if (plugin.manifest.type === 'skill' && (plugin.manifest as any).tier === 'atomic' && !options.force) {
    const allPlugins = await loader.listPlugins();
    const dependents = allPlugins.filter(p => p.tier === 'compound' && (p as any).composes?.includes(name));
    if (dependents.length > 0) {
      throw new Error(`Warning: '${dependents[0].name}' depends on '${name}'. Use --force to remove anyway.`);
    }
  }
 ...
 */
export async function listPlugins(options: { format?: string, project?: boolean } = {}) {
  const loader = new PluginLoader({ projectDir: options.project ? process.cwd() : undefined });
  const plugins = await loader.listPlugins({ includeDisabled: options.project });
...
export async function togglePlugin(name: string, enabled: boolean) {
  ...
  if (!fs.stat(path.join(projectRoot, ".gwrkrc.json")).catch(() => null)) {
    throw new Error("Not a gwrk project. Run 'gwrk init' to add gwrk to this project.");
  }
  ...
  if (config.disable.length === 0 && (!config.override || Object.keys(config.override).length === 0)) {
    ...
    new Command("disable")
      ...
      .action(async (name) => {
        await withSignal("plugin disable", async () => {
          ...
          console.log(`${GREEN}Disabled plugin '${name}' for this project.${RESET}`);
        });
      })
  )
  .addCommand(

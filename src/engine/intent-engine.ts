...
export class IntentEngine {
   ...
   */
  async executeIntents(intents: JsonIntent[], projectRoot: string): Promise<IntentSummary[]> {
  ...
  private async executeIntent(intent: JsonIntent, projectRoot: string): Promise<IntentSummary> {
    ...
    switch (intent.action) {
      case 'WRITE_FILE': {
        ...
        if (!targetPath.startsWith(absoluteRoot)) {
          throw new Error('Workflow execution violation: File writes must be within project root.');
        }
      ...
      case 'CREATE_DIR': {
        ...
        if (!targetPath.startsWith(absoluteRoot)) {
          throw new Error('Workflow execution violation: Directory creation must be within project root.');
        }
      ...
      case 'RUN_COMMAND': {
        ...
        if (
          ...
          intent.command.includes('sudo')
           ...
           throw new Error('Unsafe command execution: root level mutation or escalation blocked.');
        }
        
        try {
          const { stdout, stderr } = await execAsync(intent.command, { cwd: absoluteRoot });

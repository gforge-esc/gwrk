...
_Generated by gwrk ship_`;
        ...
        try {
          ...
          if (createMsg.includes("No commits between") || createMsg.includes("same as base branch")) {
            // Code is already on develop — nothing to PR. This is success.
            console.log("    ✓ No diff between branches — code already on develop. Merging branch.");
            try {
              execSync(`git checkout develop && git merge ${branchName} && git push`, {
                cwd: this.config.cwd,
                env: { ...process.env, GWRK_SHIP: "1" },
                stdio: ["ignore", "pipe", "pipe"],
              });
  ...
  private async dispatchWithFailback(task: TaskDispatch): Promise<TaskResult> {
    ...
    let model = this.config.selectedModel;
    ...
    if (result.exitCode !== 0 && this.config.backend === "gemini" && !model) {
      ...
      for (const fbModel of failbackModels) {
        console.log(`    ⚠ Primary model failed (exit ${result.exitCode}), failing back to ${fbModel}`);
        const fbResult = await dispatchToAgent({ ...task, model: fbModel, env: { ...env, GEMINI_MODEL: fbModel } });

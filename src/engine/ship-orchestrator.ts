...
${marker}`;
  ...
  /**
   * Block until the PR's CI is green, escalating from required checks to all
   * checks before ever concluding there is nothing to wait for.
   *
   * `gh pr checks --required` exits 1 with "no **required** checks reported"
   * when the base branch has no protection rule. The old guard tested for
   * "no checks reported", which is not a substring of that, so a repo without
   * branch protection failed PR_CI instantly despite green CI.
   *
   * Simply widening the guard would be worse than the bug: gwrk would skip CI
   * on every unprotected repo and call it success. Instead:
   *
   *   required → none required? → wait on ALL checks → none at all? → skip
   *
  ...
  private async waitForChecks(prNumber: string): Promise<void> {
    ...
    try {
      ...
      const msg =
        requiredErr instanceof Error ? requiredErr.message : String(requiredErr);
      // Anything other than "there are no required checks" is a real failure.
      if (!msg.includes("no required checks reported")) throw requiredErr;
      console.log(

...
describe("compileContext", () => {
  ...
  beforeEach(() => {
    ...
    fs.mkdirSync(path.join(tempDir, ".agent/prompts/personas"), { recursive: true });
    fs.mkdirSync(path.join(tempDir, "specs/feature-001/.gwrk"), { recursive: true });
    
    fs.writeFileSync(path.join(tempDir, ".agent/rules/coding-style.md"), "Keep it clean.");
    fs.writeFileSync(path.join(tempDir, ".agent/prompts/personas/default.md"), "Be helpful.");
    fs.writeFileSync(path.join(tempDir, "specs/feature-001/spec.md"), "# Spec\nDo this.");
    fs.writeFileSync(path.join(tempDir, "specs/feature-001/plan.md"), "# Plan\nStep 1.");
    fs.writeFileSync(path.join(tempDir, "specs/feature-001/.gwrk/tasks.json"), '{"tasks": []}');
  });

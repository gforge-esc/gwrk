 ...
 */
export async function buildProjectContext(projectRoot: string): Promise<string> {
  ...
  for (const feature of features) {
    ...
    try {
      ...
      const completed = state.phases.reduce((acc, p) => acc + p.tasks.filter(t => t.status === "completed").length, 0);

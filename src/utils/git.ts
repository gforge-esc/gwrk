...
export function gitLineCount(repoPath: string, ref: string): number {
  try {
    ...
    const stdout = execFileSync(
      "bash",
      ["-c", `git ls-tree -r ${ref} --name-only | grep -v 'png\\|jpg\\|jpeg\\|gif\\|webp\\|mp4\\|mov' | xargs -I {} git show ${ref}:"{}" 2>/dev/null | wc -l`],
 ...
 */
export function gitDraftLineCount(repoPath: string, defaultBranch: string): number {
  try {
 ...
 */
export function getDiffStats(repoPath: string, ref = "HEAD~1"): { filesChanged: number; linesAdded: number; linesDeleted: number } {
  try {
